  ...
  <div id=bodyContent style="background-color:white; width:1030px; height: 1000px;">
    ...
    <!-- Support 페이지 -->
      <div class="w3-panel w3-padding w3-xlarge" style="background-color:white;color:black; width:1030px; margin:0px;max-height:150px">
          <h2 class="w3-padding"><b>ORDER</b></h2>
      </div>
      ...
      <div class="w3-padding" style="position: relative; width:1030px; height: 800px;">
        <div style="margin-left:32px; margin-top:32px;">
          <form action="order_management.php" method="get">
            <div style="display: inline-block"><input name="searchName" type="text" class="w3-round" placeholder="Order number"><button><img src="img/search_icon.png" type="submit"/></button></div>
        ...
        <table class="w3-table w3-bordered" style="width:950px; margin-top: 16px; border-top-style: solid; border-top-color: #ff8400; border-bottom-style: solid; border-bottom-color: #ff8400; margin-left:32px; margin-right:32px;">
          <tr>
            ...
            <th style="width: 130px; text-align: center">주문번호</th>
            <th style="width: 130px; text-align: center">주문일</th>
            <th style="width: 130px; text-align: center">주문자</th>
          </tr>
          ...
          // $result2 = $mysqli->query($query2);
            ...
            $query = "SELECT * FROM orders where order_no like '%$searchName%'  ORDER BY order_no desc limit $page, $view_total";
            $result = $mysqli->query($query);
            $cnt = 0;
          ...
          ?>
          <tr onclick="location.href='order_info.php?name=<?= $data['order_no'] ?>&cnt=<?=$cnt ?> '" style="cursor:pointer">
            <td style="text-align: center">  <?php echo  "$i"; ?></td>
            <?php $text=mb_strimwidth($data['order_no'], '0', '60', '...', 'utf-8'); ?>
            <td style="text-align: center">  <?php echo "$text";?>
            ...
            <?php $timestamp = $data['orddate'];
            $u_idx = $data['user_no'];
            $query3 = "SELECT admin_id FROM admin WHERE idx = '$u_idx'";
