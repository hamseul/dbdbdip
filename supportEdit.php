...
<div class="" style="margin-left:250px">
  ...
  <!-- 여기부터 본문 레이아웃 폼 -->
  <div id=bodyContent style="background-color:#f4f7fa; height: 867px; width: 1030px">
  ...
  <!-- 0713 수정  -->
  <div style="position: relative; margin: 32px 0px 0px 32px; width: 1000px; height: 700px;">
  <?php
  include_once("db_config.php");
  ...
  $num = $_GET['name'];
  $query = "SELECT * FROM wow_support where no = $num";
  $result = $mysqli->query($query);
  $data = mysqli_fetch_array($result)
   ?>
	    <form name="nse" enctype='multipart/form-data' action="supportEdit2.php?name=<?=$num?>" method="post">
	      <input type='text' name='title'size='30' value="<?=$data[title]?>" style="width: 900px"/>
	      <textarea name="ir1" id="ir1" class="nse_content" style="width: 900px; height: 500px"> <?=$data[content]?></textarea>
